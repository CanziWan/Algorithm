...
/*
方法1：
step1 由示例1和2可以知道，x小于0或x不为0且最后一位为0时，x肯定不是回文数，返回false。
step2 创建一个数组array，将x的每一位数保存到数组中，最后用for循坏判断x是否为回文数。

方法2：
step1 由示例1和2可以知道，x小于0或x不为0且最后一位为0时，x肯定不是回文数，返回false。
step2 将x反转，并将反转后的结果保存到y中。（注意y要用long类型，用int类型会溢出）
step3 判断x于y是否相等，若相等，则x为回文数；否则，x不是回文数。
*/
